...
export default function FutureScrapbook({ onBack, onContinue }: FutureScrapbookProps) {
  ...
  const [heartPx, setHeartPx] = useState(64);

  const canPlay = !win && !paid;

  const computeHeartPx = (areaW: number, areaH: number) => {
    const base = Math.min(areaW, areaH) * 0.18;
    return clamp(Math.round(base), 42, 78);
  };
  ...
  const placeCenter = () => {
    ...
    const size = computeHeartPx(rect.width, rect.height);
    setHeartPx(size);
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      setHeartPos((prev) => {
        ...
        const maxX = Math.max(pad, rect.width - size - pad);
        const maxY = Math.max(pad, rect.height - size - pad);
        return { x: clamp(prev.x, pad, maxX), y: clamp(prev.y, pad, maxY) };
      });
    };
    ...
    <section className="fp" aria-label="Catch the Heart">
      ...
      <div className="fp__card">
        <header className="fp__header">
          ...
          <p className="fp__subtitle">Tap the heart 10 times to unlock your surprise</p>

          <div className="fp__scoreRow">
            <div className="fp__score">
        ...
        <div className="fp__gameWrap" aria-label="Game box">
